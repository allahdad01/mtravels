...
if ($type === 'ticket' || $type === 'both') {
    $ticketQuery = "SELECT id, CONCAT(description, ' (', sold, ' ', currency, ')') as description, sold, currency FROM ticket_bookings WHERE sold_to = ?";
...
if ($type === 'visa' || $type === 'both') {
    $visaQuery = "SELECT id, CONCAT(remarks, ' (', sold, ' ', currency, ')') as description, sold, currency FROM visa_applications WHERE sold_to = ?";

...
// Note: enforce_client_auth() is already called automatically in security.php
...
if (!$umrahId) {
    ...
    $umrahQuery = "SELECT ub.*,
        ...
        WHERE ub.booking_id = ? AND ub.tenant_id = ? AND ub.branch_id = ? AND ub.sold_to = ?";
    $stmt = $pdo->prepare($umrahQuery);
    $stmt->execute([$umrahId, $tenant_id, $branch_id, $client_id]);
...
<div class="page">
  ...
  <div class="topbar anim">
    ...
    <a href="umrah.php" class="btn-back">
      <i class="fa fa-arrow-left"></i> Back to Umrah
    </a>
  </div>

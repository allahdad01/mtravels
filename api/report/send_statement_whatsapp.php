...
require_once __DIR__ . '/../whatsapp/WhatsAppManager.php';

header('Content-Type: application/json');

try {
    $phone      = $_POST['phone'] ?? '';
    $entityType = $_POST['entityType'] ?? 'client';
    ...
    if (empty($phone) || empty($entityId) || empty($startDate) || empty($endDate)) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM branches WHERE tenant_id = ? AND id = ?");
    $stmt->execute([$tenant_id, $branch_id]);
    $branchDetails = $stmt->fetch(PDO::FETCH_ASSOC);

    // Get transactions (same query as share_statement.php)
    if ($entityType === 'client') {
        $transactionsQuery = "
            SELECT 
                CASE 
    ...
    $pdf = new DocPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
    ...
    $pdf->SetTextColor(...$colors['primary']);
    $pdf->Cell($columnWidth - 10, 6, strtoupper($entityType) . ': ' . strtoupper($entityDetails['name'] ?? 'N/A'), 0, 1, 'R');
    ...
    $filename = $safeName . '_Statement_' . date('Y-m-d') . '.pdf';
    $filepath = sys_get_temp_dir() . '/' . $filename;
    $pdf->Output($filepath, 'F');

    // Send via WhatsApp
    $whatsapp = new WhatsAppManager($tenant_id);
    $result = $whatsapp->sendDocument($phone, $filepath, $message, $filename);

...
try {
    ...
    $stmt = $conn->prepare("
        ...
        LEFT JOIN suppliers s ON dc.supplier = s.id AND s.tenant_id = ? AND s.branch_id = ?
        LEFT JOIN clients c ON dc.sold_to = c.id AND c.tenant_id = ? AND c.branch_id = ?
        LEFT JOIN main_account ma ON dc.paid_to = ma.id AND ma.tenant_id = ? AND ma.branch_id = ?
        LEFT JOIN users u1 ON dc.created_by = u1.id AND u1.tenant_id = ? AND u1.branch_id = ?
        LEFT JOIN users u2 ON dc.approved_by = u2.id AND u2.tenant_id = ? AND u2.branch_id = ?

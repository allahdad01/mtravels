...
$settingsQuery = $pdo->prepare("SELECT * FROM settings WHERE tenant_id = ?");
$settingsQuery->execute([$tenant_id]);
$settings = $settingsQuery->fetch(PDO::FETCH_ASSOC);
$agencyName = $settings['agency_name'] ?? 'Travel Agency';
$agencyLogo = '../uploads/' . ($settings['logo'] ?? 'assets/images/logo.png');
...
");
$stmt->execute([$tenant_id, ...$pilgrimIds]);
...
$mpdf = new \Mpdf\Mpdf([
    'mode' => 'utf-8',
    'format' => 'A4', // Page orientation is now Portrait
    'margin_left' => 10,
    'margin_right' => 10,
    'margin_top' => 10,
    'margin_bottom' => 10
...
<head>
    ...
    <style>
        ...
        .id-card {
            float: left;
            position: relative;
            width: 100mm; /* Adjusted for 2-column layout with gap */
            height: 50mm; /* Height for 3 rows on a portrait page */
        ...
        .id-card-validity, .id-card-guide-info {
             font-size: 6pt;
        }
        .pilgrim-name {
            font-size: 11pt;
        ...
        .passport-number {
            font-size: 10pt;
            text-align: center;
            margin: 5px 0 5px 0; /* Add more margin to bottom */
...
foreach ($pilgrims as $index => $pilgrim) {
    ...
    // Add ID card to HTML
    $html .= '
            <div class="' . $cardClass . '" style="margin-left: 2mm;">
                <div class="id-card-header">
                 ' . htmlspecialchars($idCardTitle) . '
                </div>
                <table class="id-card-body-table">
                    <tr>
                        <td style="width: 25mm;">
                        ...
                        <td style="width: 55mm; padding-left: 5mm;">
                            <div class="pilgrim-name">' . htmlspecialchars($pilgrim['name']) . '</div>
                            <div class="passport-number">' . htmlspecialchars($pilgrim['passport_number']) . '</div>
                            <div class="id-card-details">
                                ...
                                <div class="id-card-field">
                                    <strong>Group:</strong> <span>' . htmlspecialchars($groupName) . '</span>
                                </div>
                                <div class="id-card-field">
                                ...
                                <div class="id-card-field">
                                    <strong>Guide Makkah:</strong> <span>' . htmlspecialchars($guideMakkahName) . ' (' . htmlspecialchars($guideMakkahPhone) . ')</span><br>
                                    <strong>Guide Madina:</strong> <span>' . htmlspecialchars($guideMadinaName) . ' (' . htmlspecialchars($guideMadinaPhone) . ')</span>
                                </div>
                            </div>
                        </td>
                        <td style="width: 20mm; text-align: center; vertical-align: middle;">
                            <img src="' . $agencyLogo . '" alt="Agency Logo" class="agency-logo">
                        </td>
                    </tr>
                </table>
                <div class="id-card-address">
                    <div class="id-card-field">
                        <strong>Address:</strong> <span>' . htmlspecialchars($settings['address']) . '</span>
                        <br>
                        <strong>Phone:</strong> <span>' . htmlspecialchars($settings['phone']) . '</span>
                        <br>
                        <strong>Email:</strong> <span>' . htmlspecialchars($settings['email']) . '</span>
                    </div>
                </div>
                <div class="id-card-footer">
                    <div class="agency-info">
                        ' . htmlspecialchars($agencyName) . ' | Valid: ' . date('d M Y', strtotime($validFrom)) . ' to ' . date('d M Y', strtotime($validUntil)) . '
                    </div>
                </div>
            </div>
    ';
    
    // Add page break after every 6 cards
    if (($index + 1) % 8 === 0 && ($index + 1) < count($pilgrims)) {
        $html .= '
        </div>
    </div>
    <div class="page">

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($umrah_id <= 0) {
        $stmt_get_umrah_id = $conn->prepare("SELECT umrah_booking_id FROM umrah_transactions WHERE id = ? AND tenant_id = ? AND branch_id = ?");
        $stmt_get_umrah_id->bind_param("iii", $transaction_id, $tenant_id, $branch_id);
    ...
    try {
        ...
        $stmt_get_transaction = $conn->prepare("SELECT payment_amount, currency, transaction_to, payment_description FROM umrah_transactions WHERE id = ? AND tenant_id = ? AND branch_id = ?");
        $stmt_get_transaction->bind_param("iii", $transaction_id, $tenant_id, $branch_id);
        ...
        $stmt_fetch_umrah = $conn->prepare("SELECT paid_to, received_bank_payment, paid FROM umrah_bookings WHERE booking_id = ? AND tenant_id = ? AND branch_id = ?");
        $stmt_fetch_umrah->bind_param("iii", $umrah_id, $tenant_id, $branch_id);
        ...
        $stmt_fetch_supplier_id = $conn->prepare("SELECT supplier_id FROM umrah_booking_services WHERE booking_id = ? AND service_type IN ('all', 'visa') AND tenant_id = ? AND branch_id = ? LIMIT 1");
        $stmt_fetch_supplier_id->bind_param("iii", $umrah_id, $tenant_id, $branch_id);
        ...
        $stmt_fetch_supplier = $conn->prepare("SELECT supplier_type FROM suppliers WHERE id = ? AND tenant_id = ? AND branch_id = ?");
        ...
        $stmt_get_booking_currency = $conn->prepare("SELECT currency FROM umrah_bookings WHERE booking_id = ? AND tenant_id = ? AND branch_id = ?");
        $stmt_get_booking_currency->bind_param("iii", $umrah_id, $tenant_id, $branch_id);
       ...
       $stmt_get_exchange_rate = $conn->prepare("SELECT exchange_rate FROM umrah_transactions WHERE id = ? AND tenant_id = ? AND branch_id = ?");
       $stmt_get_exchange_rate->bind_param("iii", $transaction_id, $tenant_id, $branch_id);
...
if ($currency !== $booking_currency) {
    if ($booking_currency === 'AFS') {
        ...
        if ($transaction_to_lower === 'bank') {
            ...
            $stmt_update_received = $conn->prepare("UPDATE umrah_bookings SET received_bank_payment = ? WHERE booking_id = ? AND tenant_id = ? AND branch_id = ?");
            $stmt_update_received->bind_param("diii", $new_received_bank_payment, $umrah_id, $tenant_id, $branch_id);
            ...
            if ($supplier_type === 'External') {
                ...
                $stmt_delete_supplier_transaction = $conn->prepare("DELETE FROM supplier_transactions WHERE reference_id = ? AND transaction_of = 'umrah' AND tenant_id = ? AND branch_id = ?");
                $stmt_delete_supplier_transaction->bind_param("iii", $transaction_id, $tenant_id, $branch_id);
                ...
                $stmt_get_balance = $conn->prepare("SELECT $balance_field FROM main_account WHERE id = ? AND tenant_id = ? AND branch_id = ?");
                $stmt_get_balance->bind_param("iii", $paid_to, $tenant_id, $branch_id);
                ...
                $stmt_update_balance = $conn->prepare("UPDATE main_account SET $balance_field = ? WHERE id = ? AND tenant_id = ? AND branch_id = ?");
                $stmt_update_balance->bind_param("diii", $new_balance, $paid_to, $tenant_id, $branch_id);
                ...
                $stmt_delete_main_transaction = $conn->prepare("DELETE FROM main_account_transactions WHERE reference_id = ? AND transaction_of = 'umrah' AND tenant_id = ? AND branch_id = ?");
                $stmt_delete_main_transaction->bind_param("iii", $transaction_id, $tenant_id, $branch_id);
            ...
            $stmt_get_transaction_date = $conn->prepare("SELECT created_at FROM main_account_transactions WHERE reference_id = ? AND transaction_of = 'umrah' AND tenant_id = ? AND branch_id = ?");
            $stmt_get_transaction_date->bind_param("iii", $transaction_id, $tenant_id, $branch_id);
            ...
            ");
            $stmt_update_subsequent->bind_param("dssiis", $payment_amount, $currency, $transaction_id, $transaction_id, $tenant_id, $branch_id);
        ...
        $stmt_update_paid->bind_param("diii", $new_paid, $umrah_id, $tenant_id, $branch_id);
        ...
        $stmt_delete_transaction = $conn->prepare("DELETE FROM umrah_transactions WHERE id = ? AND tenant_id = ? AND branch_id = ?");
        $stmt_delete_transaction->bind_param("iii", $transaction_id, $tenant_id, $branch_id);
        ...
        $stmt_delete_notification = $conn->prepare("DELETE FROM notifications WHERE transaction_id = ? AND transaction_type = 'umrah' AND tenant_id = ? AND branch_id = ?");
        $stmt_delete_notification->bind_param("iii", $transaction_id, $tenant_id, $branch_id);
        ...
        $stmt_add_notification = $conn->prepare("INSERT INTO notifications (message, recipient_role, status, created_at, tenant_id, branch_id) VALUES (?, ?, ?, NOW(), ?, ?)");
        $stmt_add_notification->bind_param("sssii", $notification_message, $recipient_role, $status, $tenant_id, $branch_id);
        ...
        ");
        $stmt_log->bind_param("iissssii", $user_id, $transaction_id, $old_values, $new_values, $ip_address, $user_agent, $tenant_id, $branch_id);
        $stmt_log->execute();
        $stmt_log->close();

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $return_origin = isset($_POST['returnOrigin']) && !empty($_POST['returnOrigin']) ? htmlspecialchars($_POST['returnOrigin']) : null;
    $return_destination = isset($_POST['returnDestination']) && !empty($_POST['returnDestination']) ? htmlspecialchars($_POST['returnDestination']) : null;
    ...
    $departure_time = isset($_POST['departureTime']) && !empty($_POST['departureTime']) ? htmlspecialchars($_POST['departureTime']) : null;
    $return_date = isset($_POST['returnDate']) && !empty($_POST['returnDate']) ? $_POST['returnDate'] : null;
    $return_departure_time = isset($_POST['returnDepartureTime']) && !empty($_POST['returnDepartureTime']) ? htmlspecialchars($_POST['returnDepartureTime']) : null;
    ...
    if (isset($_POST['flight_legs']) && !empty($_POST['flight_legs'])) {
        ...
        if (count($cleanedLegs) > 0) {
            $flight_legs = json_encode($cleanedLegs);
    ...
    if (isset($_POST['return_flight_legs']) && !empty($_POST['return_flight_legs'])) {
        ...
        if (count($cleanedReturnLegs) > 0) {
            $return_flight_legs = json_encode($cleanedReturnLegs);
            ...
            if (!empty($firstReturnLeg['origin'])) $return_origin = $firstReturnLeg['origin'];
            if (!empty($lastReturnLeg['destination'])) $return_destination = $lastReturnLeg['destination'];
            if (!empty($firstReturnLeg['date'])) $return_date = $firstReturnLeg['date'];
            if (!empty($firstReturnLeg['time'])) $return_departure_time = $firstReturnLeg['time'];
        }
    }
...
if ($supplier != $originalSupplier) {
    ...
    // Handle internal suppliers - insert transaction only (no balance updates)
        ...
        $insertSupplierTransactionQuery = "INSERT INTO supplier_transactions (supplier_id, reference_id, transaction_type, amount, balance, remarks, transaction_of, transaction_date, tenant_id, branch_id, receipt)
                                           VALUES (?, ?, 'debit', ?, 0, ?, 'ticket_sale', NOW(), ?, ?, '')";
        $stmtInsertSupplierTransaction = $pdo->prepare($insertSupplierTransactionQuery);
        $description = "Purchase for ticket: $passenger_name ($origin to $destination)";
        $stmtInsertSupplierTransaction->bindParam(1, $supplier, PDO::PARAM_INT);
        $stmtInsertSupplierTransaction->bindParam(2, $id, PDO::PARAM_INT);
    ...
    $stmtDeleteOldTransactions->execute();
        ...
        $stmtTicket->bindParam(25, $flight_legs, $flight_legs === null ? PDO::PARAM_NULL : PDO::PARAM_STR);
        $stmtTicket->bindParam(26, $return_flight_legs, $return_flight_legs === null ? PDO::PARAM_NULL : PDO::PARAM_STR);
        $stmtTicket->bindParam(27, $id, PDO::PARAM_INT);
        $stmtTicket->bindParam(28, $tenant_id, PDO::PARAM_INT);
        $stmtTicket->bindParam(29, $branch_id, PDO::PARAM_INT);

...
try {
    ...
    ");
    $getTransactionStmt->execute([$transaction_id, $refund_id, 'hotel_refund', $tenant_id, $branch_id, $tenant_id, $branch_id]);
    ...
    ");
    $deleteResult = $deleteStmt->execute([$transaction_id, $refund_id, 'hotel_refund', $tenant_id, $branch_id]);
    ...
    if ($deleteResult && $deleteStmt->rowCount() > 0) {
        ...
        if ($updateResult) {
            ...
            if ($transactionCount === 0) {
                // No more transactions, update hotel_refunds status

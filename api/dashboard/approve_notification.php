...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['notification_id'])) {
    ...
    try {
        ...
        $stmt_fetch_notification = $conn->prepare("SELECT transaction_id, transaction_type FROM notifications WHERE id = ? AND tenant_id = ? AND branch_id = ?");
        $stmt_fetch_notification->bind_param("iii", $notification_id, $tenant_id, $branch_id);
        ...
        if ($transaction_type === 'visa') {
            ...
            $stmt_fetch_visa->bind_param("iii", $transaction_id, $tenant_id, $branch_id);
            ...
            $stmt_update_visa = $conn->prepare("UPDATE main_account_transactions SET receipt = ?, description = CONCAT(description, ' | Additional Remarks: ', ?) WHERE id = ? AND tenant_id = ? AND branch_id = ?");
            $stmt_update_visa->bind_param("ssiii", $receipt_number, $remarks, $transaction_id, $tenant_id, $branch_id);
        ...
        elseif ($transaction_type === 'umrah') {
            ...
            $stmt_fetch_umrah->bind_param("iii", $transaction_id, $tenant_id, $branch_id);
            ...
            if ($result->num_rows === 0) {
                ...
                $debug_query = $conn->query("SELECT id FROM umrah_transactions WHERE id = " . intval($transaction_id) . " AND tenant_id = " . intval($tenant_id) . " AND branch_id = " . intval($branch_id));
            ...
            $stmt_fetch_umrah_app = $conn->prepare("SELECT supplier_id FROM umrah_booking_services WHERE booking_id = ? AND tenant_id = ? AND branch_id = ? LIMIT 1");
            $stmt_fetch_umrah_app->bind_param("iii", $umrah_booking_id, $tenant_id, $branch_id);
            ...
            $stmt_update_umrah = $conn->prepare("UPDATE umrah_transactions SET receipt = ?, payment_description = CONCAT(payment_description, ' | Additional Remarks: ', ?) WHERE id = ? AND tenant_id = ? AND branch_id = ?");
            $stmt_update_umrah->bind_param("ssiii", $receipt_number, $remarks, $transaction_id, $tenant_id, $branch_id);
            ...
            if ($transaction_to === 'bank') {
             ...
             elseif ($transaction_to === 'internal account') {
                ...
                $stmt_update_umrah_booking = $conn->prepare("UPDATE main_account_transactions SET receipt = ? WHERE reference_id = ? and transaction_of = 'umrah' AND tenant_id = ? AND branch_id = ?");
                $stmt_update_umrah_booking->bind_param("siii", $receipt_number, $transaction_id, $tenant_id, $branch_id);
        ...
        elseif ($transaction_type === 'ticket_sale') {
            ...
            $stmt_fetch_supplier->bind_param("iiiiii", $transaction_id, $tenant_id, $branch_id, $tenant_id, $branch_id);
            $stmt_fetch_supplier->execute();
            ...
            $stmt_update_ticket = $conn->prepare("UPDATE main_account_transactions SET receipt = ?, description = CONCAT(description, ' | Additional Remarks: ', ?) WHERE id = ? and transaction_of = 'ticket_sale' AND tenant_id = ? AND branch_id = ?");
            $stmt_update_ticket->bind_param("ssiii", $receipt_number, $remarks, $transaction_id, $tenant_id, $branch_id);
            ...
            $stmt_update_ticket = $conn->prepare("UPDATE main_account_transactions SET receipt = ?, description = CONCAT(description, ' | Additional Remarks: ', ?) WHERE id = ? and transaction_of = 'ticket_reserve' AND tenant_id = ? AND branch_id = ?");
            $stmt_update_ticket->bind_param("ssiii", $receipt_number, $remarks, $transaction_id, $tenant_id, $branch_id);
            if (!$stmt_update_ticket->execute()) {
                throw new Exception('Failed to update ticket main account transaction: ' . $stmt_update_ticket->error);
            }
        ...
        $stmt_update_ticket = $conn->prepare("UPDATE main_account_transactions SET receipt = ?, description = CONCAT(description, ' | Additional Remarks: ', ?) WHERE id = ? and transaction_of = 'ticket_refund' AND tenant_id = ? AND branch_id = ?");
        $stmt_update_ticket->bind_param("ssiii", $receipt_number, $remarks, $transaction_id, $tenant_id, $branch_id);
        if (!$stmt_update_ticket->execute()) {
            throw new Exception('Failed to update ticket main account transaction: ' . $stmt_update_ticket->error);
        }
...
$stmt_update_ticket = $conn->prepare("UPDATE main_account_transactions SET receipt = ?, description = CONCAT(description, ' | Additional Remarks: ', ?) WHERE id = ? and transaction_of = 'date_change' AND tenant_id = ? AND branch_id = ?");
$stmt_update_ticket->bind_param("ssiii", $receipt_number, $remarks, $transaction_id, $tenant_id, $branch_id);
if (!$stmt_update_ticket->execute()) {
    throw new Exception('Failed to update ticket main account transaction: ' . $stmt_update_ticket->error);
}
$stmt_update_ticket->close();
 ...
 elseif ($transaction_type === 'hotel') {
     ...
     $stmt_fetch_transaction->bind_param("iii", $transaction_id, $tenant_id, $branch_id);
     ...
     $stmt_fetch_supplier->bind_param("iii", $booking_id, $tenant_id, $branch_id);
     $stmt_fetch_supplier->execute();
    ...
    $stmt_update_ticket = $conn->prepare("UPDATE main_account_transactions SET receipt = ?, description = CONCAT(description, ' | Additional Remarks: ', ?) WHERE id = ? AND transaction_of = 'hotel' AND tenant_id = ? AND branch_id = ?");
    $stmt_update_ticket->bind_param("ssiii", $receipt_number, $remarks, $transaction_id, $tenant_id, $branch_id);
    if (!$stmt_update_ticket->execute()) {
...
elseif ($transaction_type === 'additional_payment') {
    ...
    $stmt_update_transaction = $conn->prepare("UPDATE main_account_transactions SET receipt = ?, description = ? WHERE id = ? AND transaction_of = 'additional_payment' AND tenant_id = ? AND branch_id = ?");
    $stmt_update_transaction->bind_param("ssiii", $receipt_number, $combined_description, $transaction_id, $tenant_id, $branch_id);
    ...
    $stmt_check_supplier->bind_param("iii", $reference_id, $tenant_id, $branch_id);
    ...
    if ($supplier_result->num_rows > 0) {
        ...
        $stmt_update_supplier = $conn->prepare("
            ...
            SET receipt = ?,
                remarks = ?
            WHERE id = ? AND tenant_id = ? AND branch_id = ?");
        $stmt_update_supplier->bind_param("sssii", $receipt_number, $combined_description, $supplier_transaction_id, $tenant_id, $branch_id);
...
elseif ($transaction_type === 'debtor') {
    ...
    // Update main account transaction
    $stmt_update_transaction = $conn->prepare("UPDATE main_account_transactions SET receipt = ?, description = ? WHERE id = ? AND transaction_of = 'debtor' AND tenant_id = ? AND branch_id = ?");
    $stmt_update_transaction->bind_param("ssiii", $receipt_number, $combined_description, $transaction_id, $tenant_id, $branch_id);
    if (!$stmt_update_transaction->execute()) {
    ...
    $stmt_update_debtor = $conn->prepare("UPDATE debtor_transactions SET reference_number = ?, description = ? WHERE id = ? AND tenant_id = ? AND branch_id = ?");
    $stmt_update_debtor->bind_param("sssii", $receipt_number, $combined_description, $debtor_transaction_id, $tenant_id, $branch_id);
...
elseif ($transaction_type === 'creditor') {
    ...
    // Update main account transaction
    $stmt_update_transaction = $conn->prepare("UPDATE main_account_transactions SET receipt = ?, description = ? WHERE id = ? AND transaction_of = 'creditor' AND tenant_id = ? AND branch_id = ?");
    $stmt_update_transaction->bind_param("ssiii", $receipt_number, $combined_description, $transaction_id, $tenant_id, $branch_id);
    if (!$stmt_update_transaction->execute()) {
    ...
    $stmt_update_creditor = $conn->prepare("UPDATE creditor_transactions SET reference_number = ?, description = ? WHERE id = ? AND tenant_id = ? AND branch_id = ?");
    $stmt_update_creditor->bind_param("sssii", $receipt_number, $combined_description, $creditor_transaction_id, $tenant_id, $branch_id);
...
elseif ($transaction_type === 'weight') {
    ...
    // Update main account transaction
    $stmt_update_transaction = $conn->prepare("UPDATE main_account_transactions SET receipt = ?, description = ? WHERE id = ? AND transaction_of = 'weight' AND tenant_id = ? AND branch_id = ?");
    $stmt_update_transaction->bind_param("ssiii", $receipt_number, $combined_description, $transaction_id, $tenant_id, $branch_id);
    if (!$stmt_update_transaction->execute()) {
...
elseif ($transaction_type === 'umrah_refund') {
    ...
    // Update main account transaction
    $stmt_update_transaction = $conn->prepare("UPDATE main_account_transactions SET receipt = ?, description = ? WHERE id = ? AND transaction_of = 'umrah_refund' AND tenant_id = ? AND branch_id = ?");
    $stmt_update_transaction->bind_param("ssiii", $receipt_number, $combined_description, $transaction_id, $tenant_id, $branch_id);
    if (!$stmt_update_transaction->execute()) {
...
elseif ($transaction_type === 'visa_refund') {
    ...
    // Update main account transaction
    $stmt_update_transaction = $conn->prepare("UPDATE main_account_transactions SET receipt = ?, description = ? WHERE id = ? AND transaction_of = 'visa_refund' AND tenant_id = ? AND branch_id = ?");
    $stmt_update_transaction->bind_param("ssiii", $receipt_number, $combined_description, $transaction_id, $tenant_id, $branch_id);
    if (!$stmt_update_transaction->execute()) {
...
elseif ($transaction_type === 'hotel_refund') {
    ...
    // Update main account transaction
    $stmt_update_transaction = $conn->prepare("UPDATE main_account_transactions SET receipt = ?, description = ? WHERE id = ? AND transaction_of = 'hotel_refund' AND tenant_id = ? AND branch_id = ?");
    $stmt_update_transaction->bind_param("ssiii", $receipt_number, $combined_description, $transaction_id, $tenant_id, $branch_id);
    if (!$stmt_update_transaction->execute()) {
    ...
    throw new Exception('Unsupported transaction type.');
        ...
        $stmt_update_notification = $conn->prepare("UPDATE notifications SET status = 'Read' WHERE id = ? AND tenant_id = ? AND branch_id = ?");
        $stmt_update_notification->bind_param("iii", $notification_id, $tenant_id, $branch_id);

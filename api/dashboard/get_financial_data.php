...
function getFinancialWealthData($pdo, $period, $currency, $tenant_id, $branch_id) {
    ...
    try {
        ...
        if ($currency == 'USD') {
            $accountsQuery = "SELECT SUM(usd_balance) as balance FROM main_account WHERE tenant_id = ? AND branch_id = ?";
        } elseif ($currency == 'AFS') {
            $accountsQuery = "SELECT SUM(afs_balance) as balance FROM main_account WHERE tenant_id = ? AND branch_id = ?";
        } elseif ($currency == 'EUR') {
        ...
        $supplierQuery = "SELECT SUM(CASE WHEN balance > 0 THEN balance ELSE 0 END) as balance
                         FROM suppliers WHERE currency = ? AND tenant_id = ? AND branch_id = ?";
        ...
        // Get debtor balance
        $debtorQuery = "SELECT SUM(balance) as balance FROM debtors WHERE currency = ? AND tenant_id = ? AND branch_id = ?";
        ...
        $creditorQuery = "SELECT SUM(balance) as balance FROM creditors WHERE currency = ? AND tenant_id = ? AND branch_id = ?";
        ...
        $transactionQuery = "SELECT
                           ...
                           WHERE currency = ? AND tenant_id = ? AND branch_id = ? $timeConstraint
                           GROUP BY DATE(created_at), type
                           ORDER BY DATE(created_at)";

        $stmt = $pdo->prepare($transactionQuery);
        $stmt->execute([$transactionCurrency, $tenant_id, $branch_id]);
...
// Get the data
$financialData = getFinancialWealthData($pdo, $period, $currency, $tenant_id, $branch_id);

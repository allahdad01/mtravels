...
$stmt->bind_param("siii", $status, $notification_id, $tenant_id, $branch_id);
...
if ($stmt->execute()) {
    ...
    $notification_stmt->bind_param("iii", $notification_id, $tenant_id, $branch_id);
    ...
    $log_stmt = $conn->prepare("INSERT INTO activity_log
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $log_stmt->bind_param("isisssssis",
        $user_id,
        $action,
        $table_name,
        $record_id,

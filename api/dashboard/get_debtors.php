...
try {
    switch($type) {
        case 'ticket':
            $mainQuery = "SELECT tb.id, CONCAT(tb.title, ' ', tb.passenger_name) as name, tb.pnr, tb.phone, tb.currency, tb.sold as total_amount, tb.issue_date as date FROM ticket_bookings tb JOIN clients c ON tb.sold_to = c.id WHERE c.client_type = 'agency' AND tb.tenant_id = ? AND tb.branch_id = ? AND tb.imported = 0";
            ...
            break;
        case 'ticket_reserve':
            $mainQuery = "SELECT tb.id, CONCAT(tb.title, ' ', tb.passenger_name) as name, tb.pnr, tb.phone, tb.currency, tb.sold as total_amount, tb.issue_date as date FROM ticket_reservations tb JOIN clients c ON tb.sold_to = c.id WHERE c.client_type = 'agency' AND tb.tenant_id = ? AND tb.branch_id = ? AND tb.imported = 0";
            $transactionTable = 'main_account_transactions';
            $amountField = 'amount';
            $referenceField = 'reference_id';
            ...
            case 'weight':
                $mainQuery = "SELECT tw.id, CONCAT(tb.title, ' ', tb.passenger_name) as name, tb.pnr, tb.phone, tb.currency, tw.sold_price as total_amount, tw.created_at as date FROM ticket_weights tw JOIN ticket_bookings tb ON tw.ticket_id = tb.id JOIN clients c ON tb.sold_to = c.id WHERE c.client_type = 'agency' AND tw.tenant_id = ? AND tw.branch_id = ? AND tw.imported = 0";
                $transactionTable = 'main_account_transactions';
                $amountField = 'amount';
                $referenceField = 'reference_id';
        ...
        case 'datechange':
            $mainQuery = "SELECT dc.id, CONCAT(dc.title, ' ', dc.passenger_name) as name, dc.pnr, dc.phone, dc.currency, (dc.supplier_penalty + dc.service_penalty) as total_amount, dc.created_at as date FROM date_change_tickets dc JOIN clients c ON dc.sold_to = c.id WHERE c.client_type = 'agency' AND dc.tenant_id = ? AND dc.branch_id = ? AND dc.imported = 0";
            $transactionTable = 'main_account_transactions';
            $amountField = 'amount';
            $referenceField = 'reference_id';
        ...
        case 'refunded':
            $mainQuery = "SELECT rt.id, CONCAT(rt.title, ' ', rt.passenger_name) as name, rt.pnr, rt.phone, rt.currency, rt.refund_to_passenger as total_amount, rt.issue_date as date FROM refunded_tickets rt JOIN clients c ON rt.sold_to = c.id WHERE c.client_type = 'agency' AND rt.tenant_id = ? AND rt.branch_id = ? AND rt.imported = 0";
            $transactionTable = 'main_account_transactions';
            $amountField = 'amount';
            $referenceField = 'reference_id';
        ...
        case 'visa':
            $mainQuery = "SELECT v.id, CONCAT(v.title, ' ', v.applicant_name) as name, v.passport_number as pnr, v.phone, v.currency, v.sold as total_amount, v.receive_date as date FROM visa_applications v JOIN clients c ON v.sold_to = c.id WHERE c.client_type = 'agency' and v.status != 'refunded' AND v.status != 'Cancelled' AND v.tenant_id = ? AND v.branch_id = ? AND v.imported = 0";
            $transactionTable = 'main_account_transactions';
            $amountField = 'amount';
            $referenceField = 'reference_id';
        ...
        case 'addpayment':
            $mainQuery = "SELECT ap.id, ap.payment_type as name, null as pnr, '' as phone, ap.currency, ap.sold_amount as total_amount, ap.created_at as date FROM additional_payments ap LEFT JOIN clients c ON ap.client_id = c.id WHERE (c.client_type = 'agency' OR ap.client_id IS NULL) AND ap.tenant_id = ? AND ap.branch_id = ?";
            $transactionTable = 'main_account_transactions';
            $amountField = 'amount';
            $referenceField = 'reference_id';

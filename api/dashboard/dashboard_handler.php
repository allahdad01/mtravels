...
WHERE DATE(rt.created_at) = ? AND rt.tenant_id = ? AND rt.branch_id = ?";

$stmt = $pdo->prepare($dailyRefundedQuery);
$stmt->execute([$today, $tenant_id, $branch_id]);
...
WHERE DATE(dt.created_at) = ? AND dt.tenant_id = ? AND dt.branch_id = ?";
...
FROM refunded_tickets rt
LEFT JOIN ticket_bookings tb ON rt.ticket_id = tb.id
WHERE MONTH(rt.created_at) = ? AND YEAR(rt.created_at) = ? AND rt.tenant_id = ? AND rt.branch_id = ?";

$stmt = $pdo->prepare($monthlyRefundedQuery);
$stmt->execute([$month, $year, $tenant_id, $branch_id]);
...
FROM date_change_tickets dt
LEFT JOIN ticket_bookings tb ON dt.ticket_id = tb.id
WHERE MONTH(dt.created_at) = ? AND YEAR(dt.created_at) = ? AND dt.tenant_id = ? AND dt.branch_id = ?";
...
FROM refunded_tickets rt
LEFT JOIN ticket_bookings tb ON rt.ticket_id = tb.id
WHERE YEAR(rt.created_at) = ? AND rt.tenant_id = ? AND rt.branch_id = ?";

$stmt = $pdo->prepare($yearlyRefundedQuery);
$stmt->execute([$year, $tenant_id, $branch_id]);
...
FROM date_change_tickets dt
LEFT JOIN ticket_bookings tb ON dt.ticket_id = tb.id
WHERE YEAR(dt.created_at) = ? AND dt.tenant_id = ? AND dt.branch_id = ?";
...
FROM (
    SELECT profit, currency FROM ticket_bookings WHERE DATE(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT profit, currency FROM ticket_reservations WHERE DATE(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT tw.profit, tb.currency FROM ticket_weights tw LEFT JOIN ticket_bookings tb ON tw.ticket_id = tb.id WHERE DATE(tw.created_at) = ? AND tw.tenant_id = ? AND tw.branch_id = ?
    UNION ALL
    SELECT profit, currency FROM visa_applications WHERE DATE(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT profit, currency FROM umrah_bookings WHERE DATE(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT profit, currency FROM hotel_bookings WHERE DATE(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT profit, currency FROM additional_payments WHERE DATE(created_at) = ? AND tenant_id = ? AND branch_id = ?
) as combined_sales";

$stmt = $pdo->prepare($dailyQuery);
$stmt->execute([$today, $tenant_id, $branch_id, $today, $tenant_id, $branch_id, $today, $tenant_id, $branch_id, $today, $tenant_id, $branch_id, $today, $tenant_id, $branch_id, $today, $tenant_id, $branch_id, $today, $tenant_id, $branch_id]);
...
$stmt->execute([$yesterday, $tenant_id, $branch_id, $yesterday, $tenant_id, $branch_id, $yesterday, $tenant_id, $branch_id, $yesterday, $tenant_id, $branch_id, $yesterday, $tenant_id, $branch_id, $yesterday, $tenant_id, $branch_id, $yesterday, $tenant_id, $branch_id]);
$yesterdaySales = $stmt->fetch(PDO::FETCH_ASSOC);

// Add yesterday's refunded and date change profits
$stmt = $pdo->prepare($dailyRefundedQuery);
$stmt->execute([$yesterday, $tenant_id, $branch_id]);
...
$monthlyQuery = "SELECT 
    ...
    COALESCE(SUM(CASE WHEN currency = 'AED' THEN profit ELSE 0 END), 0) as aed_profit
FROM (
    SELECT profit, currency FROM ticket_bookings WHERE MONTH(created_at) = ? AND YEAR(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT profit, currency FROM ticket_reservations WHERE MONTH(created_at) = ? AND YEAR(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT tw.profit, tb.currency FROM ticket_weights tw LEFT JOIN ticket_bookings tb ON tw.ticket_id = tb.id WHERE MONTH(tw.created_at) = ? AND YEAR(tw.created_at) = ? AND tw.tenant_id = ? AND tw.branch_id = ?
    UNION ALL
    SELECT profit, currency FROM visa_applications WHERE MONTH(created_at) = ? AND YEAR(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT profit, currency FROM umrah_bookings WHERE MONTH(created_at) = ? AND YEAR(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT profit, currency FROM hotel_bookings WHERE MONTH(created_at) = ? AND YEAR(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT profit, currency FROM additional_payments WHERE MONTH(created_at) = ? AND YEAR(created_at) = ? AND tenant_id = ? AND branch_id = ?
) as combined_sales";

$stmt = $pdo->prepare($monthlyQuery);
$stmt->execute([$month, $year, $tenant_id, $branch_id, $month, $year, $tenant_id, $branch_id, $month, $year, $tenant_id, $branch_id, $month, $year, $tenant_id, $branch_id, $month, $year, $tenant_id, $branch_id, $month, $year, $tenant_id, $branch_id, $month, $year, $tenant_id, $branch_id]);
...
$stmt->execute([
    $previousMonth, $previousMonthYear, $tenant_id, $branch_id,
    $previousMonth, $previousMonthYear, $tenant_id, $branch_id,
    $previousMonth, $previousMonthYear, $tenant_id, $branch_id,
    $previousMonth, $previousMonthYear, $tenant_id, $branch_id,
    $previousMonth, $previousMonthYear, $tenant_id, $branch_id,
...
$stmt = $pdo->prepare($monthlyRefundedQuery);
$stmt->execute([$previousMonth, $previousMonthYear, $tenant_id, $branch_id]);
...
$yearlyQuery = "SELECT 
    ...
    COALESCE(SUM(CASE WHEN currency = 'AED' THEN profit ELSE 0 END), 0) as aed_profit
FROM (
    SELECT profit, currency FROM ticket_bookings WHERE YEAR(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT profit, currency FROM ticket_reservations WHERE YEAR(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT tw.profit, tb.currency FROM ticket_weights tw LEFT JOIN ticket_bookings tb ON tw.ticket_id = tb.id WHERE YEAR(tw.created_at) = ? AND tw.tenant_id = ? AND tw.branch_id = ?
    UNION ALL
    SELECT profit, currency FROM visa_applications WHERE YEAR(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT profit, currency FROM umrah_bookings WHERE YEAR(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT profit, currency FROM hotel_bookings WHERE YEAR(created_at) = ? AND tenant_id = ? AND branch_id = ?
    UNION ALL
    SELECT profit, currency FROM additional_payments WHERE YEAR(created_at) = ? AND tenant_id = ? AND branch_id = ?
) as combined_sales";

$stmt = $pdo->prepare($yearlyQuery);
$stmt->execute([$year, $tenant_id, $branch_id, $year, $tenant_id, $branch_id, $year, $tenant_id, $branch_id, $year, $tenant_id, $branch_id, $year, $tenant_id, $branch_id, $year, $tenant_id, $branch_id, $year, $tenant_id, $branch_id]);
...
$stmt->execute([$previousYear, $tenant_id, $branch_id, $previousYear, $tenant_id, $branch_id, $previousYear, $tenant_id, $branch_id, $previousYear, $tenant_id, $branch_id, $previousYear, $tenant_id, $branch_id, $previousYear, $tenant_id, $branch_id, $previousYear, $tenant_id, $branch_id]);
$previousYearSales = $stmt->fetch(PDO::FETCH_ASSOC);

// Add previous year's refunded and date change profits
$stmt = $pdo->prepare($yearlyRefundedQuery);
$stmt->execute([$previousYear, $tenant_id, $branch_id]);
...
$recentTransactionsQuery = "SELECT * FROM (
    SELECT 'ticket' as type, pnr as reference, created_at, profit, currency, tenant_id, branch_id
    FROM ticket_bookings 
    UNION ALL
    SELECT 'visa' as type, passport_number as reference, created_at, profit, currency, tenant_id, branch_id
    FROM visa_applications
    UNION ALL
    SELECT 'weight' as type, tw.ticket_id as reference, tw.created_at, tw.profit, tb.currency, tw.tenant_id, tw.branch_id
    FROM ticket_weights tw LEFT JOIN ticket_bookings tb ON tw.ticket_id = tb.id
    UNION ALL
    SELECT 'umrah' as type, passport_number as reference, created_at, profit, currency, tenant_id, branch_id
    FROM umrah_bookings
    UNION ALL
    SELECT 'hotel' as type, order_id as reference, created_at, profit, currency, tenant_id, branch_id
    FROM hotel_bookings
    UNION ALL
    SELECT 'refund' as type, rt.pnr as reference, rt.created_at, 
           (rt.service_penalty - IFNULL(tb.profit, 0)) as profit, rt.currency, rt.tenant_id, rt.branch_id
    FROM refunded_tickets rt
    LEFT JOIN ticket_bookings tb ON rt.ticket_id = tb.id
    UNION ALL
    SELECT 'datechange' as type, dt.pnr as reference, dt.created_at, 
           (dt.service_penalty - IFNULL(tb.profit, 0)) as profit, dt.currency, dt.tenant_id, dt.branch_id
    FROM date_change_tickets dt
    LEFT JOIN ticket_bookings tb ON dt.ticket_id = tb.id
) as combined_transactions WHERE tenant_id = ? AND branch_id = ?
...
LEFT JOIN (
    ...
    FROM (
        SELECT created_at, profit, currency FROM ticket_bookings WHERE tenant_id = ? AND branch_id = ?
        UNION ALL
        SELECT tw.created_at, tw.profit, tb.currency 
        FROM ticket_weights tw
        LEFT JOIN ticket_bookings tb ON tw.ticket_id = tb.id
        WHERE tw.tenant_id = ? AND tw.branch_id = ?
        UNION ALL
        SELECT created_at, profit, currency FROM visa_applications WHERE tenant_id = ? AND branch_id = ?
        UNION ALL
        SELECT created_at, profit, currency FROM umrah_bookings WHERE tenant_id = ? AND branch_id = ?
        UNION ALL
        SELECT created_at, profit, currency FROM hotel_bookings WHERE tenant_id = ? AND branch_id = ?
        UNION ALL
        SELECT created_at, profit, currency FROM additional_payments WHERE tenant_id = ? AND branch_id = ?
    ) as combined_data 
...
LEFT JOIN (
    ...
    FROM refunded_tickets rt
    LEFT JOIN ticket_bookings tb ON rt.ticket_id = tb.id
    WHERE rt.created_at >= DATE_SUB(NOW(), INTERVAL 6 MONTH) AND rt.tenant_id = ? AND rt.branch_id = ?
...
LEFT JOIN (
    ...
    FROM date_change_tickets dt
    LEFT JOIN ticket_bookings tb ON dt.ticket_id = tb.id
    WHERE dt.created_at >= DATE_SUB(NOW(), INTERVAL 6 MONTH) AND dt.tenant_id = ? AND dt.branch_id = ?
    GROUP BY DATE_FORMAT(dt.created_at, '%Y-%m')
) datechange ON months.month = datechange.month
ORDER BY months.month";

$stmt = $pdo->prepare($monthlyStatsQuery);
$stmt->execute([$tenant_id, $branch_id, $tenant_id, $branch_id, $tenant_id, $branch_id, $tenant_id, $branch_id, $tenant_id, $branch_id, $tenant_id, $branch_id, $tenant_id, $branch_id, $tenant_id, $branch_id]);
 ...
 */
function getTopPerformersByTicketProfit($month = null, $year = null, $tenant_id = null, $branch_id = null) {
    global $pdo;
    if ($tenant_id === null) {
        $tenant_id = $_SESSION['tenant_id'];
    }
    ...
    $whereClause = "u.tenant_id = ? AND u.branch_id = ?";

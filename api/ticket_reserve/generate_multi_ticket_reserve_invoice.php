...
if (!isset($invoiceData['tickets']) || !is_array($invoiceData['tickets']) || count($invoiceData['tickets']) === 0) {
    die('No reservations selected for invoice');
}
...
if (!empty($ticketIds)) {
    ...
    $ticketsQuery = "SELECT tb.id, tb.passenger_name, tb.pnr, tb.origin, tb.destination,
                           tb.airline, tb.departure_date, tb.sold, tb.trip_type,
                           tb.return_destination, tb.return_date, tb.currency
...
// Fetch paid amounts
$paidQuery = "SELECT reference_id, currency, SUM(amount) as paid FROM main_account_transactions WHERE transaction_of = 'ticket_reserve' AND reference_id IN ($placeholders) AND type = 'credit' AND tenant_id = ? AND branch_id = ? GROUP BY reference_id, currency";
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        :root {
            ...
            /* Reservation-specific status colour */
            --reserve:   #2563eb;
            --reserve-lt:#eff6ff;
        }
        ...
        .res-table thead tr { background: var(--navy); }
        .res-table thead th {
            padding: 11px 13px;
            text-align: left;
            font-size: 10px;
        ...
        .res-table thead th:last-child  { border-radius: 0 var(--radius) 0 0; text-align: right; }
        .res-table thead th.r { text-align: right; }

        .res-table tbody tr { transition: background 0.12s; }
        .res-table tbody tr:nth-child(even) { background: var(--gray-100); }
        .res-table tbody tr:hover { background: var(--accent-lt); }
        .res-table tbody td {
            padding: 13px 13px;
        ...
        @media print {
            ...
            .res-table tbody tr:hover { background: inherit; }
        }
    </style>
</head>
<body>
    ...
    <div class="invoice">
        ...
        <div class="inv-header">
            ...
            <div class="inv-meta">
                <div class="inv-label">Reservation Invoice</div>
        ...
        <div class="reservation-notice">
            ...
            This invoice covers ticket reservations — not confirmed bookings. Seats are held pending full payment.
        </div>
            ...
            <table class="res-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php $isRound = $ticket['trip_type'] === 'round_trip'; ?>
                            <span class="trip-type-badge <?php echo $isRound ? 'round' : 'oneway'; ?>">
                                <?php echo $isRound ? '&#8646; Round Trip' : '&#8594; One Way'; ?>
                            </span>
                            ...
                            <div class="sector-sub">
                                ...
                                <?php if ($isRound && $ticket['return_destination']): ?>

...
if (!isset($invoiceData['tickets']) || !is_array($invoiceData['tickets']) || count($invoiceData['tickets']) === 0) {
    die('No visa applications selected for invoice');
}
...
if (!empty($ticketIds)) {
    ...
    $ticketsQuery = "SELECT ap.id, ap.applicant_name, ap.passport_number, ap.visa_type, ap.country,
                           ap.applied_date, ap.issued_date, ap.sold
                    FROM visa_applications ap
                    WHERE ap.id IN ($placeholders) AND ap.tenant_id = ? AND ap.branch_id = ?
                    ORDER BY ap.id";
...
// Fetch paid amounts
$paidQuery = "SELECT reference_id, currency, SUM(amount) as paid FROM main_account_transactions WHERE transaction_of = 'visa_sale' AND reference_id IN ($placeholders) AND type = 'credit' AND tenant_id = ? AND branch_id = ? GROUP BY reference_id, currency";
...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .visa-table thead tr { background: var(--navy); }
        .visa-table thead th {
            padding: 11px 13px;
            text-align: left;
            font-size: 10px;
        ...
        .visa-table thead th:first-child { border-radius: var(--radius) 0 0 0; }
        .visa-table thead th:last-child  { border-radius: 0 var(--radius) 0 0; text-align: right; }
        .visa-table thead th.r { text-align: right; }

        .visa-table tbody tr { transition: background 0.12s; }
        .visa-table tbody tr:nth-child(even) { background: var(--gray-100); }
        .visa-table tbody tr:hover { background: var(--accent-lt); }
        .visa-table tbody td {
            padding: 13px 13px;
        ...
        .visa-table tbody td.r { text-align: right; }
        ...
        @media print {
            ...
            .visa-table tbody tr:hover { background: inherit; }
        }
    </style>
</head>
<body>
    ...
    <div class="invoice">
        ...
        <div class="inv-header">
            ...
            <div class="inv-meta">
                <div class="inv-label">Visa Invoice</div>
            ...
            <table class="visa-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="td-name"><?php echo h($ticket['applicant_name']); ?></td>
                        <td><span class="passport-badge"><?php echo h($ticket['passport_number']); ?></span></td>
                        <td><span class="visa-type-badge"><?php echo h($ticket['visa_type']); ?></span></td>
                        <td>
                            <div class="country-cell">
                                &#127760; <?php echo h($ticket['country']); ?>
                            </div>
                        </td>
                        <td class="date-cell">
                            <span class="date-label">Applied</span>
                            <?php echo !empty($ticket['applied_date']) ? date('d M Y', strtotime($ticket['applied_date'])) : '—'; ?>
                        </td>
                        <td class="date-cell">
                            <span class="date-label">Issued</span>
                            <?php echo !empty($ticket['issued_date']) ? date('d M Y', strtotime($ticket['issued_date'])) : '—'; ?>
                        </td>

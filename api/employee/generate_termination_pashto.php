...
$termination_date = filter_input(INPUT_POST, 'termination_date', FILTER_DEFAULT);
if (!$termination_date) {
    $termination_date = filter_input(INPUT_GET, 'termination_date', FILTER_DEFAULT);
}
...
<head>
    <meta charset="UTF-8">
    <title>ختم قرارداد - <?php echo htmlspecialchars($settings['agency_name']); ?></title>
...
<div class="container">
    <div class="controls no-print">
        <button onclick="window.print();" class="btn">چاپ قرارداد ختم‌لیک لیک</button>
        <button onclick="window.history.back();" class="btn">برگشت</button>
    </div>

    <div class="header">
    <img src="<?php echo htmlspecialchars($logoPath); ?>" alt="لوګو">
    <h1>د <?php echo $settings['agency_name']; ?> - <?php echo htmlspecialchars($branch['name']); ?> شرکت<br>د قرارداد ختم‌لیک</h1>
    <div class="date">نېټه: <?php echo date('Y-m-d'); ?></div>
</div>

<!-- Termination Letter Body -->
<div class="agreement-body">
    <div class="personal-info">
        ته: <?php echo $user['name']; ?><br>
        دنده: <?php echo $job_title; ?><br>
        اداره/شرکت: <?php echo $settings['agency_name']; ?> - <?php echo htmlspecialchars($branch['name']); ?>
    </div>

    <div class="clause">
        <h3>موضوع: د کاري قرارداد ختمېدنه</h3>
        <p>
            په درناوي سره، تاسو ته خبر درکول کېږي چې د ادارې له خوا ستاسې د څو څو ځل وظیفوي سرغړونو، د تذکر لیک (توصیه لیک) او اخطار لیک صادرېدو باوجود، ستاسو په وظیفوي چلند کې لازمي اصلاحات نه دي راغلي.<br><br>

            بناءً، د ادارې د انضباطي پالیسیو او د کاری قرارداد د شرایطو مطابق، پریکړه شوې چې **ستاسې کاري قرارداد د <?php echo $termination_date; ?> نېټې څخه رسمآ ختم شي**.<br><br>

            هیله ده چې د قرارداد د ختمېدو تر نیټې پورې د ادارې ټول وسایل، اسناد او مسؤولیتونه تسلیم کړئ، او د تصفیې پروسه بشپړه کړئ. که تاسو لا هم کوم حقوق یا اسناد لرئ، هغه به د تصفیې له تکمیل وروسته درکړل شي.

            موږ له تاسې د تیرو همکاریو مننه کوو او راتلونکې کې درته د بریا هیله لرو.
        </p>
    </div>
    <div class="signature-section">
        ...
        <div class="signature-box">
            <div class="signature-line">
                د <?php echo $settings['agency_name']; ?> - <?php echo htmlspecialchars($branch['name']); ?> شرکت رئیس<br>
                نېټه: _________________________</p>
            </div>
        </div>
    </div>

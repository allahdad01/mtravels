...
<head>
    <meta charset="UTF-8">
    <title>اخطاریه رسمی - <?php echo htmlspecialchars($settings['agency_name']); ?></title>
...
<div class="container">
    <div class="controls no-print">
        <button onclick="window.print();" class="btn">چاپ اخطاریه رسمی</button>
        <button onclick="window.history.back();" class="btn">برگشت</button>
    </div>

    <div class="header">
        <img src="<?php echo htmlspecialchars($logoPath); ?>" alt="لوگو">
        <h1>شرکت <?php echo $settings['agency_name']; ?> - <?php echo htmlspecialchars($branch['name']); ?><br>اخطاریه رسمی</h1>
...
<div class="agreement-body">
    ...
    وظیفه: <?php echo $job_title_ikhtar; ?><br>
    اداره/شرکت: <?php echo $settings['agency_name']; ?> - <?php echo htmlspecialchars($branch['name']); ?>

    </div>

    <div class="clause">
        <h3>اخطاریه رسمی بخاطر تخلف وظیفوی</h3>
         <p>احتراماً به تعقیب توسعه‌خط که به‌خاطر تخلفاتی به شما صادر گردیده بود، متأسفانه مشاهده می‌گردد که تخلف مذکور مجدداً تکرار گردیده و یا اصلاحات لازم صورت نگرفته است.            

لذا به عنوان اخطار رسمی به اطلاع شما رسانیده می‌شود که در صورت تکرار مجدد تخلف یا عدم رعایت اصول وظیفوی، اداره حق دارد مطابق به اصول وظیفه و مقررات نافذه، اقدامات انضباطی لازم از جمله فسخ قرارداد کاری را روی‌دست گیرد.

امید است این آخرین اخطار بوده و در رفتار وظیفوی‌تان اصلاحات لازم را بیاورید.
        </p>
    </div>

...
$receipt_number = isset($_POST['receipt_number']) ? DbSecurity::validateInput($_POST['receipt_number'], 'string', ['maxlength' => 100]) : null;
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT amount, currency, type, main_account_id, created_at, exchange_rate FROM main_account_transactions WHERE id = ? AND transaction_of = 'ticket_refund' AND tenant_id = ? AND branch_id = ?");

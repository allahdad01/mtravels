...
if (!isset($invoiceData['tickets']) || !is_array($invoiceData['tickets']) || count($invoiceData['tickets']) === 0) {
    die('No refund records selected for invoice');
}
...
$ticketIds = array_map(fn($t) => intval($t['id']), $invoiceData['tickets']);
...
$ticketsQuery = "
    ...
    JOIN clients c ON c.id = rt.sold_to
    WHERE rt.id IN ($placeholders) AND rt.tenant_id = ? AND rt.branch_id = ?
    ORDER BY rt.id
";
...
foreach ($tickets as $row) {
    $totalRefundAmount += floatval($row['refund_to_passenger']);
    $totalCharges      += floatval($row['refund_supplier_penalty']) + floatval($row['refund_service_penalty']);
}

// Fetch paid (debit) amounts
$paidQuery = "SELECT reference_id, currency, SUM(amount) as paid FROM main_account_transactions WHERE transaction_of = 'ticket_refund' AND reference_id IN ($placeholders) AND type = 'debit' AND tenant_id = ? AND branch_id = ? GROUP BY reference_id, currency";
...
$includeCharges = $invoiceData['includeCharges'];
$finalTotal     = $totalRefundAmount;
...
// Column span helper
$colspanTotal = $includeCharges ? 5 : 4;
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .pill.green .pill-label { color: var(--green); }
        .pill.red   .pill-label { color: var(--red); }
        .pill-value {
            font-family: 'DM Mono', monospace;
            font-size: 15px;
            font-weight: 500;
        ...
        .ref-table thead th:first-child { border-radius: var(--radius) 0 0 0; }
        .ref-table thead th:last-child  { border-radius: 0 var(--radius) 0 0; text-align: right; }
        .ref-table thead th.r { text-align: right; }

        .ref-table tbody tr { transition: background 0.12s; }
        .ref-table tbody tr:nth-child(even) { background: var(--gray-100); }
        .ref-table tbody tr:hover { background: var(--accent-lt); }
        .ref-table tbody td {
            padding: 13px 13px;
        ...
        .ref-table tbody td.r { text-align: right; }
        ...
        .sector-sub  { font-size: 11px; color: var(--gray-400); margin-top: 3px; }
        ...
        .totals-amount {
            font-family: 'DM Mono', monospace;
            font-size: 16px;
            color: #86efac; /* light green on dark bg */
        }
        .totals-paid {
            font-family: 'DM Mono', monospace;
            font-size: 13px;
        ...
        @media print {
            ...
            .ref-table tbody tr:hover { background: inherit; }
        }
    </style>
</head>
<body>
    ...
    <div class="invoice">
        ...
        <div class="inv-header">
            <div class="inv-brand">
                <div class="company-name"><?php echo h($settings['title'] ?? 'Travel Agency'); ?></div>
            ...
            <div class="inv-meta">
                <div class="inv-label">Refund Invoice</div>
        ...
        <div class="refund-notice">
            ...
            This invoice documents refund amounts payable to the passenger after applicable penalty deductions.
        </div>
        ...
        <div class="inv-body">
            ...
            <div class="addresses">
                <div class="addr-block">
                    ...
                    <div class="addr-name"><?php echo h($settings['title'] ?? 'Travel Agency'); ?></div>
                ...
                <div class="addr-block">
                    <div class="addr-label">Refund To</div>
                    <div class="addr-name"><?php echo h($clientInfo['name']); ?></div>
                    <?php if (!empty($clientInfo['address'])): ?>
                        <div class="addr-line"><?php echo h($clientInfo['address']); ?></div>
                    <?php endif; ?>
                    <?php if (!empty($clientInfo['phone'])): ?>
                        <div class="addr-line">&#9990; <?php echo h($clientInfo['phone']); ?></div>
                    <?php endif; ?>
                    <?php if (!empty($clientInfo['email'])): ?>
                        <div class="addr-line">&#9993; <?php echo h($clientInfo['email']); ?></div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="summary-pills">
                ...
                <?php if ($includeCharges): ?>
                <div class="pill red">
                    <span class="pill-label">Total Penalties</span>
                    <span class="pill-value"><?php echo h($currency); ?> <?php echo number_format($totalCharges, 2); ?></span>
                </div>
                <?php endif; ?>
                <div class="pill green">
                    <span class="pill-label">Total Refund</span>
                    <span class="pill-value"><?php echo h($currency); ?> <?php echo number_format($totalRefundAmount, 2); ?></span>
                </div>
            </div>
            ...
            <table class="ref-table">
                <thead>
                    <tr>
                        ...
                        <th>Airline</th>
                        <?php if ($includeCharges): ?>
                        <th class="r">Penalties (<?php echo h($currency); ?>)</th>
                        <?php endif; ?>
                        <th class="r">Refund (<?php echo h($currency); ?>)</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <div class="sector-sub">
                                &#9992; DEP: <?php echo date('d M Y', strtotime($ticket['departure_date'])); ?>
                            </div>
                        </td>
                        <td>
                        ...
                        <?php if ($includeCharges): ?>
                        <td class="r charge-cell">
                            <?php echo number_format($ticket['refund_supplier_penalty'] + $ticket['refund_service_penalty'], 2); ?>
                        </td>
                        <?php endif; ?>
                        <td class="r refund-cell"><?php echo number_format($ticket['refund_to_passenger'], 2); ?></td>
                    ...
                    <!-- Totals -->
                    <tr class="totals-row">
                        <td colspan="<?php echo $colspanTotal; ?>"></td>
                        <td style="text-align:right">
                            <div class="totals-label">Net Refund Total</div>
                        </td>
                        <td class="r">
                            <div class="totals-amount"><?php echo h($currency); ?> <?php echo number_format($finalTotal, 2); ?></div>
                        </td>
                        <td class="r">
        ...
        <div class="inv-bottom">
            ...
            <div class="inv-bottom-right"><?php echo h($settings['title'] ?? 'Travel Agency'); ?></div>
        </div>

...
// Build placeholders and fetch tickets (PDO)
...
$ticketsQuery = "
    ...
    FROM
        refunded_tickets rt
    JOIN
        clients c ON c.id = rt.sold_to
    WHERE
        rt.id IN ($placeholders) AND rt.tenant_id = ? AND rt.branch_id = ?
    ORDER BY
        rt.id
";
...
// --- 4. Fetch paid amounts for refund tickets ---
...
// Calculate final total based on whether charges are included
    ...
    <div class="invoice-container">
        ...
        <div class="invoice-info">
            ...
            <div class="client-info">
                ...
                <div><?php echo htmlspecialchars($clientInfo['name']); ?></div>
                <?php if (!empty($clientInfo['address'])): ?>
                    <div><?php echo htmlspecialchars($clientInfo['address']); ?></div>
                <?php endif; ?>
                <?php if (!empty($clientInfo['phone'])): ?>
                    <div>Phone: <?php echo htmlspecialchars($clientInfo['phone']); ?></div>
                <?php endif; ?>
                <?php if (!empty($clientInfo['email'])): ?>
                    <div>Email: <?php echo htmlspecialchars($clientInfo['email']); ?></div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th class="amount">Charges (<?php echo h($currency); ?>)</th>
                    <?php endif; ?>
                    <th class="amount">Refund Amount (<?php echo h($currency); ?>)</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <br><small>DEP Date: <?php echo date('M j, Y', strtotime($ticket['departure_date'])); ?></small>
                    </td>
                    <td><?php echo htmlspecialchars($ticket['airline']); ?></td>
                    <?php if ($includeCharges): ?>
                    <td class="amount"><?php echo number_format($ticket['refund_supplier_penalty'] + $ticket['refund_service_penalty'], 2); ?></td>
                    <?php endif; ?>
                    <td class="amount"><?php echo number_format($ticket['refund_to_passenger'], 2); ?></td>
                    <td class="amount">
                        <?php
                        $paidStr = '';
                ...
                <tr class="total-row">
                    ...
                    <td class="amount"><?php echo number_format($finalTotal, 2); ?></td>
                    <td class="amount">
                        <?php
                        $totalStr = '';
                        if (!empty($totalPaid)) {

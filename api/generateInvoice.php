...
if (is_null($itemId)) {
    ...
    $insertStmt = $pdo->prepare("INSERT INTO invoices (invoice_number, client_id, type, reference_id, main_account_id, total_amount, invoice_date) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $insertStmt->execute([
        $invoiceNumber,
        $clientId,
        $invoiceType,
...
try {
    ...
    $timestamp = time();
    $fileName = "invoice_summary_{$timestamp}.pdf";
    ...
    // Return success response with file URL and content
    echo json_encode([
        'success' => true,
        'message' => 'Invoice generated successfully',
        'file' => [
            'url' => '/travelagency/uploads/invoices/' . $fileName,
            'name' => $fileName,
            'content' => base64_encode($pdfOutput) // Include PDF content for direct download
        ]
    ]);

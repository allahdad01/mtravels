...
$transaction_type = isset($_GET['type']) ? $_GET['type'] : 'main';
...
if ($transaction_type === 'client') {
    // Fetch from client_transactions table
    $query = "
        SELECT
            ct.*,
            cl.id as client_id,
    ...
    // Fetch from supplier_transactions table
    $query = "
        SELECT
            st.*,
            s.id as supplier_id,
    ...
    // Fetch from main_account_transactions table
    $query = "
        SELECT
            mat.*,
            s.id as supplier_id,
        ...
        LEFT JOIN suppliers s ON mat.reference_id = s.id AND mat.transaction_of = 'supplier_fund'
        LEFT JOIN clients cl ON mat.reference_id = cl.id AND mat.transaction_of = 'client_payment'
...
// Determine transaction title based on transaction type
$transactionTitle = '';
if ($transaction['transaction_of'] === 'supplier_fund') {
    $transactionTitle = __('supplier_funding_receipt');
} elseif ($transaction['transaction_of'] === 'client_payment') {
    $transactionTitle = __('client_payment_receipt');
} elseif ($transaction['transaction_of'] === 'transfer') {
    $transactionTitle = __('transfer_receipt');
} else {
    $transactionTitle = __('fund_receipt');
}
?>
        ...
        <div class="receipt-container">
            ...
            <div class="receipt-header">
                <div class="header-row">
                    ...
                    <div class="receipt-title"><?php echo $transactionTitle; ?></div>
            ...
            <div class="receipt-details">
                <div style="text-align: center; margin-bottom: 15px;">
                    <span class="transaction-type-badge <?= $transaction['type'] === 'credit' ? 'credit' : 'debit'; ?>">
                        <?php echo $transaction['type'] === 'credit' ? __('received') : __('paid'); ?>
                    </span>
                </div>
                ...
                <div class="detail-row">
                    <span class="detail-label"><?php echo __('transaction_type'); ?>:</span>
                    <span class="detail-value"><?php echo htmlspecialchars($transaction['transaction_of']); ?></span>
                </div>
                ...
                <?php if (!empty($transaction['supplier_name'])): ?>
                <hr>
                <h6><?php echo __('supplier_information'); ?></h6>

                <div class="detail-row">
                    <span class="detail-label"><?php echo __('supplier_name'); ?>:</span>
                    <span class="detail-value"><?php echo htmlspecialchars($transaction['supplier_name']); ?></span>
                </div>

                <?php elseif (!empty($transaction['client_name'])): ?>
                <hr>
                <h6><?php echo __('client_information'); ?></h6>

                <div class="detail-row">
                    <span class="detail-label"><?php echo __('client_name'); ?>:</span>
                ...
                <div class="detail-row">
                    <span class="detail-label"><?php echo __('remarks'); ?>:</span>
                    <span class="detail-value"><?php echo htmlspecialchars($transaction['remarks']); ?></span>
                </div>
                <?php endif; ?>
            </div>
            ...
            <div class="amount-section">
                ...
                <div class="signature-section">
                    <div class="signature-box">
                        <div class="signature-label"><?php echo __('receiver_sign'); ?></div>
                        <div class="signature-line"></div>
                    </div>
                    <div class="signature-box">
                        <div class="signature-label"><?php echo __('authorized_sign'); ?></div>
                        <div class="signature-line"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="footer-note">
                <?php echo __('thank_you_for_business'); ?><br>
                <?php if (!empty($branch)): ?>

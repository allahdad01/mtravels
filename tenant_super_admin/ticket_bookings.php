...
// Main query
$query = "SELECT tb.*, u.name as created_by_name, b.name as branch_name,
    COALESCE(r.refund_to_passenger, 0) as refund_amount,
    COALESCE(dc.supplier_penalty + dc.service_penalty, 0) as date_change_penalties,
...
$params = [$tenant_id];
if ($branch_filter !== 'all') { $query .= " AND tb.branch_id = ?"; $params[] = $branch_filter; }
if (!empty($search)) {
...
// Count
$cq = "SELECT COUNT(*) as total FROM ticket_bookings tb WHERE tb.tenant_id = ?";
$cp = [$tenant_id];
if ($branch_filter !== 'all') { $cq .= " AND tb.branch_id = ?"; $cp[] = $branch_filter; }
if (!empty($search)) {
...
.amt-dc{font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--amber);margin-top:2px}
.amt-wt{font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--blue);margin-top:2px}
...
<div class="pcoded-content">
    ...
    <div class="dash-header">
        <div>
            <h4><i class="feather icon-clipboard" style="margin-right:8px;"></i>Ticket Bookings</h4>
            <p>View and manage all passenger ticket bookings</p>
        </div>
    </div>
    ...
    <div class="dash-card">
        <div class="dash-card-head">
            <h6><span class="ico"><i class="feather icon-clipboard"></i></span>Ticket Bookings</h6>
        ...
        <div class="empty-state">
            ...
            <p>No ticket bookings found<?= !empty($search) ? ' for "'.$search.'"' : '' ?>.</p>
        </div>
        <?php endif; ?>
    </div>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="feather icon-clipboard" style="margin-right:8px;"></i>Ticket Details</h5>
...
document.querySelectorAll('.view-details').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        const sold   = parseFloat(t.sold || 0);

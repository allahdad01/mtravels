...
$client_check = $pdo->prepare("SELECT id, name FROM clients WHERE id = ? AND tenant_id = ?");
$client_check->execute([$client_id, $tenant_id]);
$client = $client_check->fetch(PDO::FETCH_ASSOC);

if (!$client) {
    echo '<div class="alert alert-danger">Client not found or access denied</div>';
    exit();
}
        ...
        <tbody>
            ...
            <tr>
                <td colspan="7" class="text-center text-muted py-4">
                    <i class="feather icon-inbox"></i> No transactions found for this client
                </td>
            </tr>
            <?php else: ?>
...
<div class="d-flex justify-content-center mt-3">
    <nav aria-label="Transaction pagination">
        <ul class="pagination pagination-sm mb-0">
            <?php if ($page > 1): ?>
                <li class="page-item">
                    <a class="page-link" href="#" onclick="loadClientTransactions(<?= $client_id ?>, 1)">
                        <i class="feather icon-chevrons-left"></i>
                    </a>
                </li>
                <li class="page-item">
                    <a class="page-link" href="#" onclick="loadClientTransactions(<?= $client_id ?>, <?= $page - 1 ?>)">
            ...
            if ($start_page > 1) {
                echo '<li class="page-item"><a class="page-link" href="#" onclick="loadClientTransactions(' . $client_id . ', 1)">1</a></li>';
                if ($start_page > 2) {
            ...
            for ($i = $start_page; $i <= $end_page; $i++) {
                echo '<li class="page-item ' . ($i == $page ? 'active' : '') . '">
                    <a class="page-link" href="#" onclick="loadClientTransactions(' . $client_id . ', ' . $i . ')">' . $i . '</a>
                </li>';
            }
            ...
            if ($end_page < $total_pages) {
                ...
                echo '<li class="page-item"><a class="page-link" href="#" onclick="loadClientTransactions(' . $client_id . ', ' . $total_pages . ')">' . $total_pages . '</a></li>';
            }
            ?>
            ...
            <?php if ($page < $total_pages): ?>
                ...
                <li class="page-item">
                    <a class="page-link" href="#" onclick="loadClientTransactions(<?= $client_id ?>, <?= $total_pages ?>)">
...
<script>
// Function to load client transactions (called from parent window)
function loadClientTransactions(clientId, page) {
    ...
    fetch('get_client_transactions.php?client_id=' + clientId + '&page=' + page)

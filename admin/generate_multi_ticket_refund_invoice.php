...
$ticketsQuery = "
    ...
    WHERE 
        rt.id IN ($placeholders) AND rt.tenant_id = ?
    ORDER BY 
        rt.id
";
...
$paidQuery = "SELECT reference_id, currency, SUM(amount) as paid FROM main_account_transactions WHERE transaction_of = 'ticket_refund' AND reference_id IN ($placeholders) AND type = 'debit' AND tenant_id = ? GROUP BY reference_id, currency";

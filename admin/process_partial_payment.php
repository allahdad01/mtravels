...
try {
    ...
    $exchange_rate_query = "SELECT usd_to_afs_rate FROM settings WHERE tenant_id = ? AND branch_id = ?";
    $exchange_result = $conn->query($exchange_rate_query, $tenant_id, $branch_id);
    $exchange_rate = $exchange_result->fetch_assoc()['usd_to_afs_rate'];
    
    // Get client's current balances
    $client_query = "SELECT usd_balance, afs_balance FROM clients WHERE id = ? AND tenant_id = ? AND branch_id = ?";
    $stmt = $conn->prepare($client_query);
    $stmt->bind_param("iii", $client_id, $tenant_id, $branch_id);
    $stmt->execute();
    $client_result = $stmt->get_result();
    $client = $client_result->fetch_assoc();
    
    // Get main account balances
    $main_account_query = "SELECT usd_balance, afs_balance FROM main_account WHERE id = ? AND tenant_id = ? AND branch_id = ?";
    ...
    $stmt->bind_param("ddiii", $new_usd_balance, $new_afs_balance, $client_id, $tenant_id, $branch_id);
    $stmt->execute();
    ...
    $stmt->bind_param("ddiii", $new_main_usd_balance, $new_main_afs_balance, $main_account_id, $tenant_id, $branch_id);
    $stmt->execute();
    ...
    $stmt->bind_param("iidssii", $client_id, $main_account_id, $usd_amount, $receipt_number, $remarks, $tenant_id, $branch_id);
    $stmt->execute();
    ...
    if ($afs_amount > 0) {
        ...
        $stmt = $conn->prepare($transaction_query);
        $stmt->bind_param("iidssii", $client_id, $main_account_id, $afs_amount, $receipt_number, $remarks, $tenant_id, $branch_id);
        $stmt->execute();
    }
    ...
    $activity_log_stmt->bind_param("iisssii", $user_id, $transaction_id, $new_values_json, $ip_address, $user_agent, $tenant_id, $branch_id);

...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM sarafi_transactions WHERE id = ? AND type = 'withdrawal' AND tenant_id = ? AND branch_id = ?");
    ...
    $stmt = $pdo->prepare("SELECT * FROM main_account_transactions WHERE reference_id = ? AND transaction_of = 'withdrawal_sarafi' AND tenant_id = ? AND branch_id = ?");
    $stmt->bindParam(1, $transaction_id, PDO::PARAM_INT);
    $stmt->bindParam(2, $tenant_id, PDO::PARAM_INT);
    $stmt->bindParam(3, $branch_id, PDO::PARAM_INT);
    $stmt->execute();
    ...
    if (!$wallet || ($wallet['balance'] - $amountDifference) < 0) {
    ...
    if (!$account || ($account['current_balance'] - $amountDifference) < 0) {
    ...
    $pdo->commit();
    echo json_encode(['success' => true, 'message' => 'Withdrawal transaction updated successfully']);
} catch (Exception $e) {
    $pdo->rollBack();

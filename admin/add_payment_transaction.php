...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($type === 'credit') {
            if ($currency === 'USD') {
                $sql = "UPDATE main_account SET usd_balance = usd_balance + ? WHERE id = ? AND tenant_id = ? AND branch_id = ?";
            } else {
                $sql = "UPDATE main_account SET afs_balance = afs_balance + ? WHERE id = ? AND tenant_id = ? AND branch_id = ?";
            }
        } else {
            if ($currency === 'USD') {
                $sql = "UPDATE main_account SET usd_balance = usd_balance - ? WHERE id = ? AND tenant_id = ? AND branch_id = ?";
        ...
        $stmt->bind_param("diiii", $amount, $main_account_id, $tenant_id, $branch_id);
        $stmt->execute();

...
try {
    ...
    $getPaymentStmt = $pdo->prepare("
        ...
        WHERE sp.id = ? AND sp.tenant_id = ? AND sp.branch_id = ? AND ma.tenant_id = ? AND ma.branch_id = ?
    ");
    $getPaymentStmt->execute([$payment_id, $tenant_id, $branch_id, $tenant_id, $branch_id]);
    ...
    $deleteTransactionStmt = $pdo->prepare("
        ...
        WHERE transaction_of = 'salary_payment' AND reference_id = ? AND tenant_id = ? AND branch_id = ?
    ");
    $deleteTransactionResult = $deleteTransactionStmt->execute([$payment_id, $tenant_id, $branch_id]);
    ...
    $deletePaymentStmt = $pdo->prepare("DELETE FROM salary_payments WHERE id = ? AND tenant_id = ? AND branch_id = ?");
    $deletePaymentResult = $deletePaymentStmt->execute([$payment_id, $tenant_id, $branch_id]);
    ...
    if ($deletePaymentResult) {
        ...
        ");
        $updateAccountResult = $updateAccountStmt->execute([$amount, $main_account_id, $tenant_id, $branch_id]);
        ...
        if ($updateAccountResult) {
            ...
            if ($payment['payment_type'] === 'regular') {
                ...
                ");
                $getAdvanceDeductionsStmt->execute([$payment['user_id'], $payment['currency'], $tenant_id, $branch_id]);
                ...
                foreach ($advances as $advance) {
                    ...
                    $updateAdvanceStmt->execute([$newAmountPaid, $newStatus, $advance['id'], $tenant_id, $branch_id]);
                }
            }

...
try {
    ...
    if ($type === 'client_supplier') {
        ...
        $paymentQuery = "SELECT jp.*,
                         ...
                         WHERE jp.id = ? AND jp.tenant_id = ? AND jp.branch_id = ?";
    } else {
    ...
    $paymentStmt->execute([$jvId, $tenant_id, $branch_id]);
    ...
    $transactionsQuery = "SELECT jt.*, DATE_FORMAT(jt.created_at, '%Y-%m-%d %H:%i') as created_at
                          ...
                          WHERE jt.jv_payment_id = ? AND jt.tenant_id = ? AND jt.branch_id = ?
                          ORDER BY jt.created_at DESC";
    $transactionsStmt = $pdo->prepare($transactionsQuery);
    $transactionsStmt->execute([$jvId, $tenant_id, $branch_id]);

...
$remaining_salary = $current_salary;

try {
    // Total regular salary payments this month
    $pay_sql = "SELECT COALESCE(SUM(amount), 0) as total FROM salary_payments WHERE user_id = ? AND tenant_id = ? AND branch_id = ? AND currency = ? AND payment_type = 'regular' AND DATE_FORMAT(payment_for_month, '%Y-%m') = ?";
    $pay_stmt = $pdo->prepare($pay_sql);
    $pay_stmt->execute([$advance_user_id, $tenant_id, $branch_id, $default_currency, $current_month]);
    $remaining_salary -= floatval($pay_stmt->fetchColumn());

    // Total advances this month (same currency)
    $adv_sql = "SELECT COALESCE(SUM(amount), 0) as total FROM salary_advances WHERE user_id = ? AND tenant_id = ? AND branch_id = ? AND currency = ? AND DATE_FORMAT(created_at, '%Y-%m') = ?";
    $adv_stmt = $pdo->prepare($adv_sql);
    $adv_stmt->execute([$advance_user_id, $tenant_id, $branch_id, $default_currency, $current_month]);
    $remaining_salary -= floatval($adv_stmt->fetchColumn());

    // Total deductions this month
    $ded_sql = "SELECT COALESCE(SUM(amount), 0) as total FROM salary_deductions WHERE user_id = ? AND tenant_id = ? AND branch_id = ? AND DATE_FORMAT(deduction_date, '%Y-%m') = ?";
    $ded_stmt = $pdo->prepare($ded_sql);
    $ded_stmt->execute([$advance_user_id, $tenant_id, $branch_id, $current_month]);
    $remaining_salary -= floatval($ded_stmt->fetchColumn());

    // Total bonuses this month
    $bon_sql = "SELECT COALESCE(SUM(amount), 0) as total FROM salary_bonuses WHERE user_id = ? AND tenant_id = ? AND branch_id = ? AND DATE_FORMAT(bonus_date, '%Y-%m') = ?";
    $bon_stmt = $pdo->prepare($bon_sql);
    $bon_stmt->execute([$advance_user_id, $tenant_id, $branch_id, $current_month]);
    $remaining_salary += floatval($bon_stmt->fetchColumn());
} catch (PDOException $e) {
    // On error, keep remaining_salary as base_salary
}
        ...
        <div class="sm-page">
            ...
            <div class="form-card">
                ...
                <div class="form-card-body">
                    ...
                    <form id="advanceForm" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . "?advance_user_id=" . $advance_user_id; ?>" method="post">
                        <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px; margin-bottom:20px">
                            <div class="field-group">
                                <label class="field-label"><?= __('select_account') ?></label>
                                <select id="main_account_id" class="field-control <?php echo (!empty($main_account_id_err)) ? 'is-invalid' : ''; ?>" name="main_account_id" required>
                            ...
                            <div class="field-group">
                                ...
                                <input type="number" id="amount" class="field-control <?php echo (!empty($amount_err)) ? 'is-invalid' : ''; ?>" name="amount" step="0.01" value="<?php echo $amount; ?>" required>
                                <?php if (!empty($amount_err)): ?>
                                <span class="field-error"><?= $amount_err ?></span>
                                <?php else: ?>
                                <span class="field-hint"><?= __('remaining_salary') ?>: <strong id="remaining-salary"><?= number_format(max(0, $remaining_salary), 2) ?></strong> <?= $default_currency ?></span>
                                <?php endif; ?>
                            </div>
                            <div class="field-group">
                                <label class="field-label"><?= __('currency') ?></label>
                                <input type="hidden" name="currency" value="<?= htmlspecialchars($default_currency) ?>">
                                <input type="text" class="field-control" value="<?= htmlspecialchars($default_currency) ?>" readonly style="background:#f4f5f7; cursor:not-allowed;">
                            </div>
                        </div>
                        <div class="field-group">
...
$(document).ready(function() {
    // Remaining salary for current month
    var remainingSalary = <?php echo max(0, $remaining_salary); ?>;
    ...
    $('#advanceForm').on('submit', function(e) {
        ...
        if (amount > remainingSalary) {
            if (!confirm('<?= __('advance_amount_exceeds_remaining_salary_confirm') ?>')) {
                e.preventDefault();
                return false;
            }
        }
    ...
    $('#main_account_id, #amount').change(function() {

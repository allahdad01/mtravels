...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($user_id_err) && empty($amount_err) && empty($description_err) && empty($bonus_date_err)) {
        ...
        $stmt->bindParam(6, $bonus_date, PDO::PARAM_STR);
...
// Fetch bonus records
try {
    $stmt = $pdo->prepare("SELECT sb.*, u.name as employee_name, a.name as added_by_name FROM salary_bonuses sb JOIN users u ON sb.user_id=u.id JOIN users a ON sb.created_by=a.id WHERE sb.tenant_id=? AND sb.branch_id=? ORDER BY sb.bonus_date DESC");
...
$total_bonus_amount = array_sum(array_map(fn($r) => floatval($r['amount']), $bonuses));
...
<div class="pcoded-main-container">
    <div class="pcoded-content">
        <div class="sm-page">
            ...
            <div class="page-hero">
                <div>
                    <div class="page-hero-title"><?= __('manage_employee_bonuses') ?></div>
                    <div class="page-hero-subtitle">Manage and track employee bonus records</div>
                </div>
            ...
            <div class="stat-grid">
                <div class="stat-card blue">
                    <div class="stat-label"><?= __('total_bonuses') ?></div>
                    <div class="stat-value"><?= $total_bonuses ?></div>
                    <div class="stat-icon">📊</div>
                </div>
                <div class="stat-card green">
                    <div class="stat-label"><?= __('total_amount') ?></div>
                    <div class="stat-value"><?= number_format($total_bonus_amount, 0) ?></div>
                    <div class="stat-icon">💰</div>
                </div>
            ...
            <div class="add-panel-wrapper" id="addPanelWrapper">
                <div class="add-panel">
                    <div class="add-panel-header">
                        <h3><?= __('add_new_bonus') ?></h3>
                    </div>
                    <div class="add-panel-body">
                        <?php if (isset($_GET['success']) && $_GET['success'] == 1): ?>
                        <div class="alert alert-success">
                            <strong><?= __('success') ?></strong> <?= __('bonus_has_been_added_successfully') ?>
                        </div>
                        <?php endif; ?>
                        ...
                        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                            <div style="display:grid; grid-template-columns:1fr 1fr; gap:18px; margin-bottom:18px">
                                ...
                                <div class="field-group">
                                    ...
                                    <input type="date" class="field-control <?php echo (!empty($bonus_date_err)) ? 'is-invalid' : ''; ?>" name="bonus_date" value="<?php echo empty($bonus_date) ? date('Y-m-d') : $bonus_date; ?>">
                                    <?php if (!empty($bonus_date_err)): ?>
                                    <div class="field-error"><?= $bonus_date_err ?></div>
                                    <?php endif; ?>
                                </div>
                            </div>
                            <div class="field-group">
                            ...
                            <div style="display:flex; gap:10px">
                                <button type="submit" class="btn-primary"><?= __('add_bonus') ?></button>
            ...
            <div class="data-table">
                <div style="padding:16px 20px; border-bottom:1px solid var(--border); display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px">
                    <div style="font-weight:600; color:var(--ink); font-size:14px"><?= __('bonus_records') ?></div>
                </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td colspan="8" class="table-empty">
                                    ...
                                    <div class="table-empty-title"><?= __('no_bonus_records') ?></div>
                                    <div class="table-empty-text"><?= __('click_add_bonus_to_get_started') ?></div>
                                </td>
                            </tr>
                            <?php else: ?>
                                ...
                                <tr data-id="<?= $bonus['id'] ?>">
                                    ...
                                    <td><strong><?= number_format($bonus['amount'], 2) ?></strong></td>
                                    <td><span style="background:rgba(61,108,255,.1); color:var(--accent); padding:4px 8px; border-radius:4px; font-size:11px; font-weight:600; text-transform:capitalize"><?= ucfirst($bonus['type']) ?></span></td>
                                    <td class="muted"><?= date('Y-m-d', strtotime($bonus['bonus_date'])) ?></td>
                                    <td class="muted"><?= substr($bonus['description'], 0, 40) . (strlen($bonus['description']) > 40 ? '...' : '') ?></td>
                                    <td class="muted"><?= $bonus['added_by_name'] ?></td>
                                    <td>
                                        <a href="edit_bonus.php?id=<?= $bonus['id'] ?>" class="btn-info" style="text-decoration:none; display:inline-block">
                                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                        </a>
                                        <a href="delete_bonus.php?id=<?= $bonus['id'] ?>" class="btn-danger" style="text-decoration:none; display:inline-block" onclick="return confirm('<?= __('are_you_sure') ?>')">
                ...
                <div class="table-footer">
                    <span id="tableCount"><?= count($bonuses) ?> record<?= count($bonuses) !== 1 ? 's' : '' ?></span>
                </div>
            </div>
...
<?php if (!empty($user_id_err) || !empty($amount_err) || !empty($description_err) || !empty($bonus_date_err)): ?>
...
document.addEventListener('DOMContentLoaded', () => showToast('<?= __('bonus_has_been_added_successfully') ?>'));

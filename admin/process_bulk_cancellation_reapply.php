...
try {
    ...
    foreach ($bookings as $booking) {
        ...
        try {
            ...
            $updateQuery = "UPDATE umrah_bookings SET profit = ?, status = ? WHERE booking_id = ? AND tenant_id = ?";
            $stmt = $conn->prepare($updateQuery);
            $stmt->bind_param('dsii', $new_profit, $new_status, $booking_id, $tenant_id);
            if (!$stmt->execute()) {
            ...
            if ($action === 'cancel') {
                ...
                $updateDueQuery = "UPDATE umrah_bookings SET due = 0 WHERE booking_id = ? AND tenant_id = ?";
                $stmt = $conn->prepare($updateDueQuery);
                $stmt->bind_param('ii', $booking_id, $tenant_id);
                $stmt->execute();
            } else {
                // Recalculate due for re-applied bookings
                $calculateDueQuery = "UPDATE umrah_bookings SET due = sold_price - paid WHERE booking_id = ? AND tenant_id = ?";
            ...
            foreach ($services as $service) {
                ...
                $updateServiceQuery = "UPDATE umrah_booking_services SET profit = ? WHERE booking_id = ? AND id = ? AND tenant_id = ?";
                $stmt = $conn->prepare($updateServiceQuery);
                $stmt->bind_param('diii', $serviceNewProfit, $booking_id, $service['id'], $tenant_id);
                $stmt->execute();
                ...
                if ($action === 'cancel' || $action === 'reapply') {
                    ...
                    if ($supplier_type === 'External') {
                        ...
                        $insertSupplierTransactionStmt->bind_param("iiddsssi",
                            $supplier_id,
                            $booking_id,
                            $supplierAmount,
                            $newSupplierBalance,
            ...
            if ($action === 'cancel' || $action === 'reapply') {
                ...
                $clientQuery->bind_param("ii", $booking_data['sold_to'], $tenant_id);
                ...
                if ($clientResult['client_type'] === 'regular') {
                    ...
                    if ($currency === 'USD') {
                        ...
                        $stmt->bind_param("dii", $newBalance, $booking_data['sold_to'], $tenant_id);
                    } else {
            ...
            try {
                $logQuery = "INSERT INTO cancellation_reapply_log (booking_id, action, base_price, sold_price, previous_profit, new_profit, reason, tenant_id, created_by, created_at)
                             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
                $stmt = $conn->prepare($logQuery);
                $action_type = ($action === 'cancel') ? 'cancellation' : 'reapplication';
                $stmt->bind_param('issddssii', $booking_id, $action_type, $base_price, $sold_price, $current_profit, $new_profit, $reason, $tenant_id, $_SESSION['user_id']);
                $stmt->execute();
            } catch (Exception $e) {
            ...
            try {
                ...
                $activity_log_stmt->bind_param("isssssi",
                    $_SESSION['user_id'],
                    $activity_action,
                    $booking_id,
                    $old_values,

...
$exchange_rate = isset($_POST['exchange_rate']) ? DbSecurity::validateInput($_POST['exchange_rate'], 'float', ['min' => 0]) : null;
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT amount, currency, type, main_account_id, created_at, exchange_rate FROM main_account_transactions WHERE id = ? AND transaction_of = 'ticket_refund' AND tenant_id = ?");
        ...
        $stmt->bind_param("dssdii", $newAmount, $newDescription, $newDate, $exchange_rate, $transactionId, $tenant_id);

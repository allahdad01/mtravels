...
$stmt = $pdo->prepare("SELECT id, name, base_salary, salary_type FROM sales_agents WHERE user_id = ?");
...
// Check if agent has salary
if ($agent['salary_type'] === 'commission') {
    // Redirect commission-only agents to dashboard
...
// Count total salary payments from sales_agent_salary_payments table
$stmt = $pdo->prepare("SELECT COUNT(*) as total FROM sales_agent_salary_payments WHERE sales_agent_id = ? AND status IN ('paid', 'processing')");
$stmt->execute([$agent_id]);
$total_result = $stmt->fetch();
$total_items = $total_result['total'] ?? 0;

if ($total_items === 0) {
    // If no salary payments, show message
    $salary_payments = [];
    $total_pages = 1;
} else {
    ...
    $stmt = $pdo->prepare("SELECT * FROM sales_agent_salary_payments 
                           WHERE sales_agent_id = ? AND status IN ('paid', 'processing')
...
$stmt = $pdo->prepare("SELECT 
                       ...
                       ORDER BY year DESC");
$stmt->execute([$agent_id]);
$yearly_breakdown = $stmt->fetchAll();
?>
...
<style>
  ...
  .info-box {
    background: var(--info-light); border: 1px solid var(--info); color: var(--info);
    border-radius: var(--radius-sm); padding: 16px; margin-bottom: 20px; font-size: .85rem;
  }
</style>
      ...
      <div class="pcoded-inner-content">
            ...
            <div class="row align-items-center">
              <div class="col-md-12">
                <div class="page-header-title">
                  <h5 class="m-b-10">Salary Payments</h5>
                </div>
                <ul class="breadcrumb">
                  <li class="breadcrumb-item"><a href="dashboard.php"><i class="feather icon-home"></i></a></li>
                  <li class="breadcrumb-item"><a href="#!">Salary Payments</a></li>
                </ul>
              </div>
            </div>
          </div>
        ...
        <div class="main-body">
          <div class="page-wrapper sa-dashboard">
            ...
            <!-- ── Base Salary ── -->
            <p class="section-label">Your Salary</p>
            <div class="info-box">
              <strong>Monthly Base Salary:</strong> $<?= number_format($agent['base_salary'], 2) ?><br>
              <small>Salary payments are typically processed on the last business day of each month</small>
            </div>
            ...
            <div class="metric-grid">
              ...
              <div class="metric-card green">
                ...
                <div class="metric-value">$<?= number_format($salary_summary['total_paid'] ?? 0, 2) ?></div>
                <div class="metric-label">Total Paid</div>
                <div class="metric-sub">All salary payments</div>
              </div>
              ...
              <div class="metric-card blue">
                ...
                <div class="metric-value"><?= $salary_summary['total_payments'] ?? 0 ?></div>
                <div class="metric-label">Payments</div>
                <div class="metric-sub">Total salary payments</div>
              </div>
              ...
              <div class="metric-card amber">
                ...
                <div class="metric-value">$<?= number_format($salary_summary['processing'] ?? 0, 2) ?></div>
                <div class="metric-label">Processing</div>
                <div class="metric-sub">Awaiting transfer</div>
              </div>
              ...
              <div class="metric-card cyan">
                ...
                <div class="metric-value"><?= $salary_summary['latest_payment'] ? date('M d', strtotime($salary_summary['latest_payment'])) : 'N/A' ?></div>
                <div class="metric-label">Latest Payment</div>
                <div class="metric-sub"><?= $salary_summary['latest_payment'] ? date('Y', strtotime($salary_summary['latest_payment'])) : '' ?></div>
              </div>
            </div>
                ...
                <?php if (!empty($salary_payments)): ?>
                  <?php foreach ($salary_payments as $payment): ?>
                    <div class="list-card-item">
                      ...
                      <div class="lci-body">
                        <div class="lci-title"><?= date('F Y', strtotime($payment['payment_date'])) ?></div>
                        <div class="lci-sub">Payment date: <?= date('M d, Y', strtotime($payment['payment_date'])) ?></div>
                      </div>
                      <div class="lci-right">
                        <div class="lci-amount">$<?= number_format($payment['amount'], 2) ?></div>
                        <span class="pill pill-<?= $payment['status'] ?>"><?= ucfirst($payment['status']) ?></span>
                      </div>
                    </div>
                  <?php endforeach; ?>
                <?php else: ?>
                  <p style="text-align:center; color:var(--text-muted); padding: 24px 0; margin:0; font-size:.88rem;">No salary payments recorded yet.</p>
                <?php endif; ?>
              </div>
            ...
            <div class="card-saas">
              <div class="card-saas-body" style="text-align: center; padding: 40px 20px;">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="48" height="48" style="margin: 0 auto 16px; color: var(--text-muted);"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                <h6 style="color: var(--text-primary); margin-bottom: 8px;">No Salary Payments Yet</h6>
                <p style="color: var(--text-muted); font-size: .85rem; margin: 0;">Salary payments will appear here once they are processed by the admin.</p>
              </div>
            </div>
            <?php endif; ?>

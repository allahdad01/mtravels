...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_pricing') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("
                UPDATE settings
                SET monthly_price = ?, quarterly_price = ?, yearly_price = ?
                WHERE tenant_id = ?
            ");
...
$query = "
    SELECT t.id, t.name, t.subdomain, t.status,
        s.monthly_price, s.quarterly_price, s.yearly_price,
        ts.currency
    FROM tenants t
...
.price-unset { color: var(--muted); font-weight: 400; }
.sa-pill {
...
.sa-pill.deleted { background: #f3f4f6; color: #6b7280; }
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-6">
                            <h5 class="mb-0">
                                ...
                                Tenant Addon Pricing
                            </h5>
                            <p class="mb-0 mt-1" style="font-size:14px;opacity:0.9">Manage pricing for additional branch add-ons per tenant</p>
                        </div>
                    </div>
                </div>
                <!-- [ breadcrumb ] end -->
                    ...
                    <div class="page-wrapper">
                        ...
                        <div class="sa-table-wrap">
                            <div class="sa-toolbar">
                                ...
                                <span style="font-size:.8rem;color:var(--muted)">Total: <?= $total_items ?> tenants</span>
                            </div>

                            <div style="padding:12px 20px;border-bottom:1px solid var(--border)">
                                <form method="GET" action="manage_tenant_addon_pricing.php" style="display:flex;gap:8px;flex-wrap:wrap">
                                    <input type="text" class="sa-form-control" name="search" placeholder="Search tenant name or subdomain..." value="<?= htmlspecialchars($search_query) ?>" style="max-width:320px">
                                    <button type="submit" class="sa-btn" style="padding:8px 14px">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                                        Search
                                    </button>
                                    <?php if (!empty($search_query)): ?>
                                    <a href="manage_tenant_addon_pricing.php" class="sa-btn-ghost" style="padding:8px 14px">Clear</a>
                                    <?php endif; ?>
                                </form>
                            </div>
                            ...
                            <table class="sa-table">
                                ...
                                <tbody>
                                    <?php foreach ($tenants as $tenant):
                                        ...
                                        $monthly = $tenant['monthly_price'] ? $symbol . number_format($tenant['monthly_price'], 2) : null;
                                        $quarterly = $tenant['quarterly_price'] ? $symbol . number_format($tenant['quarterly_price'], 2) : null;
                                        $yearly = $tenant['yearly_price'] ? $symbol . number_format($tenant['yearly_price'], 2) : null;
                                    ?>
                                    <tr>
                                        <td>
                                        ...
                                        <td class="price-cell"><?= $monthly ?? '<span class="price-unset">Not set</span>' ?></td>
                                        <td class="price-cell"><?= $quarterly ?? '<span class="price-unset">Not set</span>' ?></td>
                                        <td class="price-cell"><?= $yearly ?? '<span class="price-unset">Not set</span>' ?></td>
                                        <td class="sa-td-actions">
                                            <button type="button" class="sa-btn sa-btn-ghost" style="padding:6px 12px"
                                                onclick="openEditModal(<?= $tenant['id'] ?>, '<?= htmlspecialchars($tenant['name'], ENT_QUOTES) ?>', <?= $tenant['monthly_price'] ?? 50 ?>, <?= $tenant['quarterly_price'] ?? 150 ?>, <?= $tenant['yearly_price'] ?? 600 ?>, '<?= htmlspecialchars($tenant['currency'] ?? 'USD') ?>')">
                            ...
                            <div class="empty-state">
                                ...
                                <p style="font-size:.85rem"><?= !empty($search_query) ? 'Try adjusting your search filters.' : 'No tenants available for pricing configuration.' ?></p>
                            </div>
                            <?php endif; ?>
...
function openEditModal(tenantId, tenantName, monthly, quarterly, yearly, currency) {
    ...
    var sym = currency === 'AFN' ? '؋' : currency === 'EUR' ? '€' : currency === 'GBP' ? '£' : '$';
    document.getElementById('currency_symbol_monthly').textContent = sym;
    document.getElementById('currency_symbol_quarterly').textContent = sym;
    document.getElementById('currency_symbol_yearly').textContent = sym;

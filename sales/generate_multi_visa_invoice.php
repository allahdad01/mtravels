...
if (!empty($ticketIds)) {
    ...
    $ticketsQuery = "SELECT ap.id, ap.applicant_name, ap.passport_number, ap.visa_type, ap.country, ap.applied_date, ap.issued_date, ap.sold
                    ...
                    WHERE ap.id IN ($placeholders) AND ap.tenant_id = ?
                    ORDER BY ap.id";
...
$paidQuery = "SELECT reference_id, currency, SUM(amount) as paid FROM main_account_transactions WHERE transaction_of = 'visa_sale' AND reference_id IN ($placeholders) AND type = 'credit' AND tenant_id = ? GROUP BY reference_id, currency";
